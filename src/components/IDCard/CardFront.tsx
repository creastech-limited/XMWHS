...
function QRDisplay({ data, size = 80 }: { data: string; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: "block" }}>
      ...
      {grid.map((row, ri) =>
        row.map((on, ci) => {
          const isCorner = (ri < 3 && ci < 3) || (ri < 3 && ci >= cells - 3) || (ri >= cells - 3 && ci < 3);
          if (isCorner) return null;
          return on ? (
            <rect key={`${ri}-${ci}`} x={o + ci * cellSize} y={o + ri * cellSize} width={cellSize - 0.6} height={cellSize - 0.6} fill="#000" />
          ) : null;
        })
      )}
      {corner(o, o)}
...
// Card is 54mm × 85.6mm — at 96dpi that's ~204 × 323px
// We render at 2× (408 × 646px) for sharpness, then scale down visually with transform
const CARD_W = 204;
const CARD_H = 323;
const SCALE = 2; // render at 2× for crisp PDF capture
...
const CardFront = React.forwardRef<HTMLDivElement, CardFrontProps>(({ student }, ref) => {
  ...
  useEffect(() => {
    if (student.qrData && !student.qrData.startsWith("data:") &&
  ...
  // The outer wrapper is the ACTUAL visual size (204×323)
  // The inner div renders at 2× and is scaled down with transform
  return (
    <div
      style={{
        width: CARD_W,
    ...
    >
      {/* Inner 2× render — captured by html2canvas at full res */}
      <div
        ref={ref}
        style={{
          width: RW,
      ...
      >
        ...
        >
          ...
          {/* Email */}
          <p style={{ fontSize: 12, color: "#555", margin: "0 0 8px", textAlign: "center" }}>
            {student.email}
          </p>

          {/* Scan Me + QR */}
          <div style={{ display: "flex", alignItems: "center", gap: 5, alignSelf: "flex-start", paddingLeft: 4, marginBottom: 4 }}>
            <span style={{ fontSize: 11, color: "#555" }}>Scan Me</span>
            <svg width="24" height="20" viewBox="0 0 24 20" fill="none">
              <circle cx="5" cy="10" r="2.2" fill="#555" />
              <path d="M9 10 Q9 6 13.5 6" stroke="#555" strokeWidth="2" fill="none" strokeLinecap="round" />
              <path d="M9 10 Q9 2.5 17 2.5" stroke="#555" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.6" />
              <path d="M9 10 Q9 -1 21 -1" stroke="#555" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.3" />
            </svg>
          </div>
