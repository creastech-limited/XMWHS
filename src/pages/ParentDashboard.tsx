...
const ParentDashboard: React.FC = () => {
  ...
  const prepareTrendData = (): TrendData[] => {
    const monthlyData = transactions.reduce((acc: Record<string, {payment: number, transaction: number}>, transaction) => {
  ...
  // Data for charts
  const trendData = transactions.length > 0 ? prepareTrendData() : [
  ...
  const spendingCategories = transactions.length > 0 ? prepareSpendingCategories() : [
  ...
  const quickActions: QuickAction[] = [
    { icon: <CreditCardIcon className="w-8 h-8" />, label: 'Fund Wallet', color: 'bg-indigo-900', route: '/fundwallet' },
    { icon: <UsersIcon className="w-8 h-8" />, label: 'Transfer to Kids', color: 'bg-indigo-800', route: '/transfertokids' },
    { icon: <AcademicCapIcon className="w-8 h-8" />, label: 'Pay School Bills', color: 'bg-blue-600', route: '/payschoolbills' },
    { icon: <DocumentTextIcon className="w-8 h-8" />, label: 'Pay to Agent', color: 'bg-blue-700', route: '/paytoagent' }
  ];
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="w-4/5 max-w-xl">
          <p className="mb-4 text-center text-lg font-medium">Loading your dashboard...</p>
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-xl text-center">
          ...
          <p className="text-gray-700">Please try refreshing the page or log in again.</p>
          <Link 
            to="/login"
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex flex-grow gap-6">
         ...
         <div className="fixed top-24 left-0 h-[calc(100vh-6rem)] z-20">
  <Psidebar />
</div>


           {/* Main Content Area - Add dynamic padding based on sidebar state */}
          <div className="flex-grow transition-all duration-300 pl-64">
              {/* Quick Actions Section */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {quickActions.map((action, index) => (
                  <Link key={index} to={action.route} className="block">
                    <div className={`${action.color} text-white rounded-xl shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-lg`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-blue-400 text-white rounded-xl shadow-md p-5 relative overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <p className="text-sm">Transfer money to your kids and pay school bills.</p>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                <div className="lg:col-span-2 bg-white rounded-xl shadow-md p-5">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-medium text-gray-800">Payment & Transaction Trends</h3>
                  ...
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={trendData}>
                        ...
                        />
                        <Legend />
                        <Bar dataKey="payment" fill="#1a237e" radius={[4, 4, 0, 0]} name="Payment" />
                        <Bar dataKey="transaction" fill="#f59e0b" radius={[4, 4, 0, 0]} name="Transaction" />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                        ...
                        <div className="flex justify-between items-center py-3">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                              activity.category === 'credit' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'
                            }`}>
                            ...
                            <div>
                              <p className="font-medium text-gray-800">
                                {activity.description || (activity.category === 'credit' ? 'Credit' : 'Debit')}
                              </p>
                          ...
                          <div className="text-right">
                            <p className={`font-semibold ${
                              activity.category === 'credit' ? 'text-emerald-600' : 'text-rose-600'
                            }`}>
                              {activity.category === 'credit' ? '+' : '-'}₦{activity.amount.toLocaleString()}
                            </p>
