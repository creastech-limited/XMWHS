...
const TransactionModule: React.FC = () => {
  ...
  const exportTransactions = () => {
    ...
    const csvContent = "data:text/csv;charset=utf-8," + 
      ...
      transactions.map(t => `${t.id},${t.student},${t.amount},${t.date},${t.status}`).join("\n");
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="flex flex-grow">
        ...
        <main className="flex-grow p-6">
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold text-black">Fee Transactions</h2>
                <button
                    ...
                    {Array.from({ length: 10 }).map((_, index) => {
                      ...
                      return (
                        <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                          ...
                          <td className="p-3">₦{(Math.random() * 100000 + 5000).toFixed(2)}</td>
                          <td className="p-3">{new Date(2023, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1).toLocaleDateString()}</td>
                          <td className="p-3">
          ...
          {snackbar.open && (
            <div className={`fixed bottom-4 right-4 ${snackbar.severity === 'success' ? 'bg-green-600' : 'bg-red-600'} text-white px-4 py-3 rounded-lg shadow-lg flex items-center z-50`}>
