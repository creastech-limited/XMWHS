...
const ManageAgentsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        // Fetch agents and count (no store ID needed as parameter)
        await fetchAgents();
  ...
  // Function to fetch agents - no storeId parameter needed
  const fetchAgents = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/users/getagentsbyid`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
        }
  ...
  // Function to fetch agent count - no storeId parameter needed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!storeDetails || !storeDetails.store_id) {
    ...
    try {
      ...
      // Refresh agents list (no store ID parameter needed)
      await fetchAgents();
      await fetchAgentCount();
