...
const KidsDashboard = () => {
  ...
  // Navigation items
  const navItems = [
    { label: "Dashboard", icon: <Wallet className="w-5 h-5" />, route: "/kidswallet" },
    { label: "Pay Agent", icon: <WalletCards className="w-5 h-5" />, route: "/kidpayagent" },
    { label: "History", icon: <History className="w-5 h-5" />, route: "/kidpaymenthistory" },
    { label: "School Bills", icon: <SchoolIcon className="w-5 h-5" />, route: "/schoolbills" },
  ...
  const processAnalyticsData = (transactionData: Transaction[]) => {
    console.log("Processing analytics for", transactionData.length, "transactions");
    if (transactionData && transactionData.length > 0) {
      ...
      const categoryAnalytics = Object.entries(categories).map(([name, value], index) => ({
      ...
      const monthlyAnalytics: Record<string, { credits: number, debits: number }> = {};
      ...
      const monthlyChartData = Object.entries(monthlyAnalytics).map(([name, data]) => ({
        name,
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-gray-50">
        <div className="w-full max-w-md p-6">
          <h2 className="text-xl font-semibold text-center mb-4">Loading your dashboard...</h2>
  ...
  const renderDashboardContent = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white transform hover:-translate-y-1 transition">
        ...
        <p className="text-3xl font-bold my-3">₦{wallet?.balance.toLocaleString() || '0'}</p>
          ...
          <>
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip formatter={(value: number) => `₦${value.toLocaleString()}`} />
                    <Legend />
                    <Line type="monotone" dataKey="credits" stroke="#4caf50" activeDot={{ r: 8 }} name="Credits" />
                    <Line type="monotone" dataKey="debits" stroke="#f44336" name="Debits" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            {(analytics.length === 0 || categoryData.length === 0) && (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-gray-400 text-sm mt-2">Complete some transactions to see analytics.</p>
              </div>
            )}
          </>
        )}
  ...
  <div key={tx._id} className="flex items-center justify-between p-3 hover:bg-gray-100 rounded-lg transition border border-gray-200"> {/* Changed hover:bg-gray-50 to hover:bg-gray-100 */}
    <div className="flex items-center">
      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
        tx.category === 'credit' || tx.amount > 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
      }`}>
      ...
      <div className="ml-3">
        <p className="font-medium text-gray-800">{getTransactionDisplayName(tx)}</p> {/* Added text-gray-800 */}
        <p className="text-sm text-gray-600"> {/* Changed text-gray-500 to text-gray-600 */}
          {tx.description} • {new Date(tx.createdAt).toLocaleDateString()}
        </p>
      </div>
    </div>
    <div className="text-right">
      <p className={`font-bold ${tx.category === 'credit' || tx.amount > 0 ? 'text-green-600' : 'text-red-600'}`}>
        {tx.category === 'credit' || tx.amount > 0 ? '+' : '-'}₦{Math.abs(tx.amount).toLocaleString()}
      </p>
  ...
  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow hover:shadow-md transition flex items-center gap-2"
>
  ...
  const renderPayAgentContent = () => (
    <div className="flex justify-center">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-6 text-center">
        <h3 className="text-xl font-semibold text-blue-600 mb-2">Pay to Agent</h3>
        <p className="text-gray-600 mb-6">
          Show this QR code to any authorized payment agent to send or receive money.
        </p>
        <div 
          ref={qrRef} 
      ...
      <div className="max-w-6xl mx-auto flex flex-col min-h-screen">
        ...
        <div className="flex-1 mb-8">
          {activeTab === 0 ? renderDashboardContent() : 
           activeTab === 1 ? renderPayAgentContent() : 
